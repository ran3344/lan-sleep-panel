...
- `启动休眠服务.bat`
- `打包发布.bat`：生成适合上传到 GitHub Releases 的 ZIP 发布包
...
如果你想给普通用户提供“直接下载就用”的压缩包，可以运行：

- `打包发布.bat`

它会自动在 `dist/` 目录下生成一个 ZIP 文件，内容只包含运行项目需要的文件，不会把这些本地内容打进去：
...
生成好的 ZIP 很适合直接上传到 GitHub 的 `Releases` 页面。
